...
def callback_mocap(odometry_msg):
    ...
    old_ranges_global_discrete = np.zeros((720,2))
    ...
    for i in range(diff.shape[0]):
        if not diff[i, :] == 0 or not diff[i, :] == 0:
            nonzero_indices.append(i)

    dynamic_distance = curr_scan.ranges
    dynamic_distance = list(dynamic_distance)
    for i in range(len(dynamic_distance)):
        if not i in nonzero_indices:
            dynamic_distance[i] = float("inf")

    dynamic_scan = LaserScan()
    dynamic_scan.ranges=dynamic_distance
...
def main():
    #TODO: change to frequency
    rate = rospy.Rate(39) # 30 [Hz]
